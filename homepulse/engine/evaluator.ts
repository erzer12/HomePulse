...
export function evaluateTriage(input: TriageInput): TriageOutput {
  ...
  const { state: modifiedState, modifiers } = applyHouseholdModifiers(baseState, input);
...
function evaluateBaseState(input: TriageInput): 1 | 2 | 3 | 4 {
  ...
  if (
    ...
    return 3;
  if (temp > CLINICAL_THRESHOLDS.baseStateModerateFeverCelsius || symptom.hydration_status === 'poor') return 2;
...
function buildState(level: 1 | 2 | 3 | 4): ActionState {
  const states: Record<1 | 2 | 3 | 4, ActionState> = {
    1: {
      ...
      explanation: 'Symptoms are mild and not worsening. Your household is capable of safe observation.',
      triggers: [],
      redFlags: ['Fever rises above 39°C', 'Breathing becomes difficult', 'Person becomes confused'],
    ...
    2: {
      ...
      triggers: [],
      redFlags: ['Temperature exceeds 39.5°C', 'Unable to keep fluids down', 'Breathing worsens'],
      recheckIntervalMinutes: 120,
    },
    3: {
      level: 3,
      label: 'Arrange Teleconsultation',
      explanation: 'Symptoms need a clinical assessment. Arrange a teleconsult or clinic visit within a few hours.',
      triggers: [],
      redFlags: ['Any breathing difficulty', 'Confusion or unresponsiveness', 'Severe worsening'],
      recheckIntervalMinutes: 240,
    },
    4: {
      level: 4,
      label: 'Seek Urgent Care Now',
      explanation: 'This situation requires immediate in-person medical attention. Go to the nearest clinic or hospital now.',
      triggers: [],
      redFlags: [],
