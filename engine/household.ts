...
export function applyHouseholdModifiers(
  ...
  if (!h.overnight_caregiver && (age_group === 'elderly' || age_group === 'infant')) {
    adjusted = Math.min(adjusted + 1, 4);
    modifiers.push('No overnight caregiver — elevated monitoring required');
  }

  if (!h.has_thermometer) {
    modifiers.push('No thermometer — cannot verify fever trend; conservative guidance applied');
  }

  return { state: Math.max(1, Math.min(4, adjusted)) as 1 | 2 | 3 | 4, modifiers };
}
