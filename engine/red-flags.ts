...
export function checkRedFlags(input: TriageInput): RedFlagResult {
  ...
  if (['confused', 'unresponsive'].includes(symptom.consciousness)) {
    return { triggered: true, reason: 'Altered consciousness detected — seek immediate care' };
  }

  if (symptom.breathing_difficulty) {
    return { triggered: true, reason: 'Breathing difficulty — cannot safely monitor at home' };
  }
