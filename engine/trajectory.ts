...
export function analyzeTrajectory(
	...
	if (
		...
		current.temperature_celsius >
			previous.temperature_celsius +
				CLINICAL_THRESHOLDS.trajectoryTemperatureRiseCelsius
	) {
	...
	if (
		...
		current.spo2_percent <
			previous.spo2_percent - CLINICAL_THRESHOLDS.trajectorySpo2DropPercent
	) {
	...
	if (
		hydrationScore[current.hydration_status] >
		hydrationScore[previous.hydration_status]
	) {
	...
	if (
		consciousnessScore[current.consciousness] >
		consciousnessScore[previous.consciousness]
	) {
	...
	return {
		worsening:
			dimensions_worsened.length >=
			CLINICAL_THRESHOLDS.trajectoryEscalationDimensions,
		dimensions_worsened,
	};
}
