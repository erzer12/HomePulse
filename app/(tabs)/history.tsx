...
const MOCK_HISTORY = [
  {
    ...
    notes: 'Fever has increased since morning. Administered Paracetamol. Child is slightly more lethargic.',
  },
  {
    id: '2',
  ...
  const renderItem = ({ item, index }: { item: typeof MOCK_HISTORY[0], index: number }) => (
    <MotionView delay={index * DURATION.stagger} style={styles.entryContainer}>
      ...
      <View style={styles.timelineLineContainer}>
        ...
        <View style={[styles.timelineDot, { backgroundColor: item.state.theme.primary }]} />
      </View>

      <Card variant="elevated" style={styles.entryCard}>
        <View style={styles.entryHeader}>
          <Text style={styles.timestamp}>{item.timestamp}</Text>
          <View style={[styles.badge, { backgroundColor: item.state.theme.surface }]}>
        ...
        <View style={styles.vitalsRow}>
          {item.vitals.map((vital, i) => (
            <View key={i} style={styles.vitalItem}>
              <Text style={styles.vitalLabel}>{vital.label}</Text>
              <View style={styles.vitalValueContainer}>
                <Text style={styles.vitalValue}>{vital.value}</Text>
                {vital.trend === 'up' && <TrendingUp size={16} color={COLORS.state.urgent.primary} />}
                {vital.trend === 'down' && <TrendingDown size={16} color={COLORS.state.monitor.primary} />}
                {vital.trend === 'stable' && <Minus size={16} color={COLORS.textSecondary} />}
              </View>
            </View>
          ))}
        </View>
